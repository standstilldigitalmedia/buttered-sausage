...
func apply_animations(severity: ButteredSausageDisplay.Severity) -> void:
	...
	match severity:
		ButteredSausageDisplay.Severity.SUCCESS:
			animator.configure(ButteredSausageAnimator.Axis.VERTICAL, ButteredSausageAnimator.OpenDirection.POSITIVE, panel_width) \
				.with_speed(0.4) \
				.with_fade() 
...
func apply_config(config: Dictionary = {}) -> void:
	if not config.is_empty():
		...
		if config.has("panel_width"): panel_width = config.panel_width
