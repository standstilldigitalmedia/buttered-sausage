...
class_name ButteredSausageToastDisplay
extends Control
...
func clear_all_panels() -> void:
	for child in content_container.get_children():
		if child is ButteredSausageToastPanel:
			child.dismiss()
	hide()
...
func _remove_panel_immediately(panel: ButteredSausageToastPanel) -> void:
...
func _enforce_panel_limit(new_severity: int) -> bool:
	...
	var panels: Array[ButteredSausageToastPanel] = []
	for child in content_container.get_children():
		if child is ButteredSausageToastPanel:
	...
	if max_visible_panels == 1:
		...
		var highest_priority_panel: ButteredSausageToastPanel = null
	...
	while panels.size() >= max_visible_panels:
		...
		var oldest_panel: ButteredSausageToastPanel
...
func create_message(msg: String, severity: int) -> void:
	...
	var panel: ButteredSausageToastPanel
	match severity:
		SSDMSeverity.Level.SUCCESS:
			panel = success_panel.instantiate() as ButteredSausageToastPanel
		SSDMSeverity.Level.ERROR:
			panel = error_panel.instantiate() as ButteredSausageToastPanel
		SSDMSeverity.Level.WARNING:
			panel = warning_panel.instantiate() as ButteredSausageToastPanel
		SSDMSeverity.Level.INFO:
			panel = info_panel.instantiate() as ButteredSausageToastPanel
...
func _ready() -> void:
	...
	ButteredSausageToastPanel.cached_styles.clear()
